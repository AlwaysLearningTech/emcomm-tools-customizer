...
### Preseed & Installer Automation
**The ISO uses Ubiquity (GUI installer on live system) with preseed for automated installation.**

**Key Facts**:
- **Boot system**: Casper (live boot from squashfs) + Ubiquity (GUI installer)
- **Preseed file**: `/preseed.cfg` in ISO root (accessed via `file=/cdrom/preseed.cfg`)
- **Boot parameter for auto-install**: `only-ubiquity` (skips live desktop, boots directly to installer)
- **Without `only-ubiquity`**: Boots to live desktop, user must click "Install Ubuntu" manually
- **Preseed provides**: User credentials, timezone, keyboard, partitioning hints (limited)

**CRITICAL: Ubuntu Desktop ISO Architecture**:
- Ubuntu Desktop ISOs do NOT use debian-installer (d-i) directly
- They boot Casper → live session → Ubiquity installer
- Preseed with d-i directives works via Ubiquity's d-i compatibility layer
- Some d-i directives are ignored (netcfg, LVM/RAID, tasksel)

...
customize_preseed() {
    ...
    if ! mkdir -p "$preseed_dir"; then
        log "ERROR" "[PRESEED] Failed to create preseed directory: $preseed_dir"
        log "ERROR" "[PRESEED] Check permissions and available disk space"
        return 1
    fi
    log "DEBUG" "[PRESEED] Preseed directory created successfully"
    ...
    log "DEBUG" "[PRESEED] Creating preseed file with cat heredoc..."
    cat > "$preseed_file" <<'EOF'
# Ubuntu 22.10 Preseed File for Automated Installation with Ubiquity
# Generated by EmComm Tools Customizer
# Reference: https://wiki.ubuntu.com/UbiquityAutomation
#
# IMPORTANT: This preseed is for Ubiquity (GNOME desktop installer).
# Boot with "automatic-ubiquity" kernel parameter to activate automatic mode.
# In automatic mode, ubiquity will respect preseed values and not show prompts.
...
# Account setup (USER account - required for ubiquity)
...
# NOTE: These d-i partition directives are IGNORED by ubiquity.
# Ubiquity uses a guided partitioner and won't honor these exact settings.
# These are kept for reference but ubiquity will ask user or use defaults.
...
# Ubiquity-specific directives (these are ACTUALLY USED)
# Suppress the summary page
ubiquity ubiquity/summary text

# Suppress the release notes page
...
# Mirror configuration for package installation
...
# Success command - run after installation
ubiquity ubiquity/success_command string /root/post-install-setup.sh

# Package selection (tasksel directives are IGNORED by ubiquity)
# Ubiquity installs what's on the CD by default
...
update_grub_for_preseed() {
    ...
    # Reference: https://wiki.ubuntu.com/UbiquityAutomation
    # 
    # Ubiquity (GNOME desktop installer) requires:
    #   1. file=/cdrom/preseed/custom.preseed  (preseed file location)
    #   2. automatic-ubiquity                  (ENABLES AUTOMATIC MODE - respects preseed)
    #   3. noprompt                           (skip "remove disc" prompt at end)
    #
    # Without "automatic-ubiquity", ubiquity ignores the preseed and asks questions.
    # The "automatic-ubiquity" boot parameter is ESSENTIAL - it's what makes ubiquity
    # respect the preseed file and not show interactive prompts.
    
    # Use sed to replace preseed file and boot parameters in linux commands
    sed -i 's|file=/cdrom/preseed/ubuntu\.seed maybe-ubiquity|file=/cdrom/preseed/custom.preseed automatic-ubiquity noprompt maybe-ubiquity|g' "$grub_cfg"
    
    # Also handle cases where preseed is not already specified
    if ! grep -q "file=/cdrom/preseed/custom.preseed" "$grub_cfg"; then
        sed -i 's|/casper/vmlinuz|/casper/vmlinuz file=/cdrom/preseed/custom.preseed automatic-ubiquity noprompt|g' "$grub_cfg"
    fi
    
    log "DEBUG" "GRUB config updated for ubiquity preseed auto-installation"
    
    # Verify the change took effect
    if grep -q "file=/cdrom/preseed/custom.preseed" "$grub_cfg" && grep -q "automatic-ubiquity" "$grub_cfg"; then
        ...
        log "WARN" "GRUB config update may have failed - verify preseed and automatic-ubiquity are present"
    fi
